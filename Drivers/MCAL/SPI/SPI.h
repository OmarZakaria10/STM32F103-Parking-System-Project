...
typedef struct
{
	...
	SPI_TypeDef 	 *SPIx;               // Specifies SPI number
	...
	uint16_t 	TXE_INT_EN :1;				// TX Buffer Empty Interrupt

	uint16_t	RXNE_INT_EN :1;				//RX Buffer Not Empty Interrupt
	uint8_t		ERRI_INT_EN :1;				// Error Interrupt

	void (* SPI1_IRQ_CallBack)(void);	//set the C Function which will be called once IRQ Happens
	void (* SPI2_IRQ_CallBack)(void);	//set the C Function which will be called once IRQ Happens
