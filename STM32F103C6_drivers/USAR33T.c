...
void MCAL_UART_Init (USART_TypeDef *USARTx, UART_Config* UART_Config){
	...
	{
		UART_Config->USARTx->CR1 |= (UART_Config->IRQ_Enable) ;
...
void MCAL_UART_SendData	(UART_Config* UART_Config, uint16_t *pTxBuffer,enum Polling_mechism PollingEn )
{
	...
	if (Global_UART_Config->Payload_Length == UART_Payload_Length_9B)
	{
		...
		//			bit.
		//
		UART_Config->USARTx->DR = (*pTxBuffer & (uint16_t)0x01FF);
...
void MCAL_UART_ReceiveData	(USART_TypeDef *USARTx, uint16_t *pRxBuffer ,enum Polling_mechism PollingEn )
{
	...
	if (Global_UART_Config->Payload_Length == UART_Payload_Length_9B)
	{
		if (Global_UART_Config->Parity ==UART_Parity__NONE)
		{
			...
			*((uint16_t*) pRxBuffer) = UART_Config->USARTx->DR ;

		}else
		{
			//Parity is used, so, 8bits will be of user data and 1 bit is parity
			*((uint16_t*) pRxBuffer) = ( UART_Config->USARTx->DR  & (uint8_t)0xFF );
	...
	}else
	{
		...
		}else
		{
			...
			*((uint16_t*) pRxBuffer) = ( UART_Config->USARTx->DR  & (uint8_t)0X7F );
...
void MCAL_UART_GPIO_Set_Pins (USART_TypeDef *USARTx)
{
	...
	if ( UART_Config->USARTx == USART1 )
	{
		...
		PinCfg.GPIO_MODE = GPIO_ALT_OUTPUT_PUSHPULL_MODE;
		...
		if (Global_UART_Config->HwFlowCtl  ==UART_HwFlowCtl_CTS  || Global_UART_Config->HwFlowCtl  ==UART_HwFlowCtl_RTS_CTS  ){
			...
			PinCfg.GPIO_MODE = GPIO_FLOATING_INPUT_MODE;
			MCAL_GPIO_Init(GPIOA, &PinCfg);
		}
